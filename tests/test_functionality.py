...
def test_bot_functionality():
    ...
    try:
        ...
        for username, message, should_respond, expected_reason in test_cases:
            ...
            if result == should_respond and (not should_respond or reason == expected_reason):
                print(f"✅ Test '{username}': '{message}' -> {result} ({reason})")
            else:
                print(f"❌ Test '{username}': '{message}' -> Expected {should_respond}, got {result}")
...
def test_chat_scenarios():
    ...
    try:
        ...
        for scenario in scenarios:
            result, reason = detector.should_respond(scenario["username"], scenario["message"])
