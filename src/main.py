...
from utils.config import setup_logging, load_config, validate_config, get_startup_message
...
class TwitchAIBot:
    ...
    def _initialize_handlers(self) -> bool:
        ...
        try:
            ...
            self.twitch_handler.initialize_bot(message_callback=self._on_message_received)
            ...
            if not self.chat_handler.initialize():
                self.logger.error("❌ Échec de l'initialisation du gestionnaire de chat")
                return False
    ...
    async def _send_startup_message(self):
        ...
        try:
            if self.chat_handler:
                startup_msg = await self.chat_handler.get_startup_personality_announcement()
                await self.twitch_handler.send_message(startup_msg)
                self.logger.info(f"Message de démarrage envoyé: {startup_msg}")
            else:
                fallback_msg = "🤖 nova_the_red_cat est connecté ! Prêt à être sarcastique... 😏"
