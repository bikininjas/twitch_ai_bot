...
class ChatHandler:
    ...
    def __init__(self, ai_handler, twitch_handler):
        ...
        self.event_listener = TwitchEventListener(self.event_handler, self._send_message_callback)
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Erreur lors de l'initialisation du gestionnaire de chat: {e}")
            return False
    ...
    async def _send_message_callback(self, message: str):
        ...
        try:
            if self.twitch_handler:
                ...
                logger.error("Gestionnaire Twitch non disponible pour envoyer le message")
        except Exception as e:
    ...
    async def _on_personality_change_event(self, event_type: str, force: bool = False) -> Optional[str]:
        """
        ...
        try:
            if self.ai_handler:
                ...
                if announcement:
                    logger.info(f"Personnalité changée suite à l'événement: {event_type}")
                    return announcement
                else:
                    logger.debug(f"Changement de personnalité ignoré (cooldown) pour: {event_type}")
                    return None
            else:
                return "🤖 Quelque chose d'excitant s'est passé, mais je suis confus ! 🤯"
                
        except Exception as e:
            logger.error(f"Erreur lors du changement de personnalité pour événement {event_type}: {e}")
            ...
            if self.ai_handler:
                ...
                try:
                    ...
                    if current_personality:
                        ...
                        import random
                        announcement_variant = random.choice(current_personality.announcement_variants)
                ...
                return "🤖 nova_the_red_cat est connecté ! Prêt à être sarcastique... 😏"
    ...
    def get_status(self) -> dict:
        ...
        status = {
            ...
            "ai_handler_status": self.ai_handler.get_status() if self.ai_handler else None,
            "twitch_handler_status": self.twitch_handler.get_connection_status() if self.twitch_handler else None
        }
    ...
    def test_response_generation(self, test_username: str = "testuser", 
                               test_message: str = "Hello bot!") -> bool:
        """
        ...
        """
        try:
            trigger_info = self.mention_detector.get_trigger_info(test_username, test_message)
