...
class TwitchEventHandler:
    ...
    def __init__(self, personality_change_callback: Callable[[str], None]):
        ...
        self.cooldown_time = (
            300  # 5 minutes entre changements pour le même type d'événement
        )

        logger.info("Gestionnaire d'événements Twitch initialisé")

    async def handle_subscription(
        self, subscriber_name: str, tier: str = "1000", months: int = 1
    ):
        """
        ...
        try:
            ...
            if not self._is_event_on_cooldown(event_type):
                logger.info(
                    f"Nouvel abonnement détecté: {subscriber_name} (Tier {tier}, {months} mois)"
                )
    ...
    async def handle_gift_subscription(
        self, gifter_name: str, recipient_name: str, tier: str = "1000"
    ):
        """
        Gère un gift sub
        ...
        try:
            ...
            if not self._is_event_on_cooldown(event_type):
                logger.info(
                    f"Gift sub détecté: {gifter_name} -> {recipient_name} (Tier {tier})"
                )
    ...
    async def handle_manual_change(self, trigger_user: str = "manuel"):
        ...
        try:
            ...
            if not self._is_event_on_cooldown(event_type):
                logger.info(
                    f"Changement manuel de personnalité déclenché par: {trigger_user}"
                )
    ...
    async def _trigger_personality_change(
        self, event_type: str, trigger_user: str, event_details: str
    ) -> str:
        """
        ...
        try:
            ...
            force_change = (
                event_type != "manual"
            )  # Forcer sauf pour les changements manuels
            personality_announcement = await self.personality_change_callback(
                event_type, force_change
            )
            ...
            logger.info(
                f"Changement de personnalité déclenché par {event_type}: {trigger_user}"
            )
...
class TwitchEventListener:
    ...
    def __init__(
        self, event_handler: TwitchEventHandler, message_sender: Callable[[str], None]
    ):
        """
        ...
        try:
            ...
            if "followed" in content or "following" in content:
                ...
                if message.author.name != "redpikpik":  # Pas nous-mêmes
                    event_msg = await self.event_handler.handle_follow(
                        message.author.name
                    )
                    if event_msg:
            ...
            # Messages de sub (détection par pattern)
                ...
                event_msg = await self.event_handler.handle_subscription(
                    message.author.name, tier="1000"  # Par défaut
                )
                if event_msg:
                    await self.message_sender(event_msg)
            ...
            # Messages de bits (détection par pattern)
                ...
                if bits_match:
                    ...
                    event_msg = await self.event_handler.handle_cheer(
                        message.author.name, bits
                    )
                    if event_msg:
                        await self.message_sender(event_msg)
        ...
        try:
            event_msg = await self.event_handler.handle_subscription(
                ...
                (
                    ...
                    if hasattr(subscription, "cumulative_months")
                    else 1
                ),
            )
            if event_msg:
