    ...
    def _load_personalities(self) -> Dict[str, Personality]:
        ...
        try:
            ...
            if not os.path.exists(personalities_dir):
                logger.error(f"Dossier des personnalités introuvable : {personalities_dir}")
                ...
                if filename.endswith('.json'):
                    ...
                    try:
                        ...
                        announcement_variants = [
                            f"{data['emoji']} {phrase} ! Mode gaming activé ! {data['emoji']}" 
                            for phrase in data['iconic_phrases'][:3]
                        ]
                        
                        personality = Personality(
                            name=data['name'],
                            type=personality_type,
                            description=data.get('description', f"Personnalité basée sur {data['cultural_figure']}"),
                            prompt_base=f"""Tu es {data['name']}, un bot Twitch qui parle comme {data['cultural_figure']} !
...
Utilise les expressions iconiques : {', '.join(data['iconic_phrases'])}
...
- Évite les phrases trop longues""",
                            signature=f"- nova {data['emoji']}",
                            announcement_variants=announcement_variants,
                            color_emoji=data['emoji'],
                            catchphrases=data['iconic_phrases']
                        )
                        personalities[personality_type] = personality
                        logger.debug(f"Personnalité chargée : {personality_type} -> {data['name']}")
                        
                    except Exception as e:
                        logger.error(f"Erreur lors du chargement de {filename} : {e}")
                        continue
            
            logger.info(f"Chargé {len(personalities)} personnalités depuis {personalities_dir}")
...
- Sois sarcastique mais amusant !""",
            signature="- nova 🐱",
            announcement_variants=["🎭 Mode sarcasme activé ! Préparez-vous aux vannes ! 😏"],
            color_emoji="🔴",
    ...
    def _set_default_personality(self):
        ...
        if self.personalities:
            default_key = "sarcastic" if "sarcastic" in self.personalities else list(self.personalities.keys())[0]
    ...
    def get_random_personality(self, exclude_current: bool = True) -> Personality:
        ...
        return random.choice(available) if available else self.current_personality
    
    def change_personality(self, new_personality: Optional[str] = None, force: bool = False) -> Tuple[bool, str]:
    ...
    def get_current_signature(self) -> str:
        ...
        if not self.current_personality:
            self._set_default_personality()
        return self.current_personality.signature if self.current_personality else "- nova 🐱"
    
    def get_current_personality_name(self) -> str:
        """Retourne le nom de la personnalité actuelle"""
        if not self.current_personality:
            self._set_default_personality()
        return self.current_personality.name if self.current_personality else "nova_the_red_cat"
